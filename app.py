...
FEATURE_COLS = ['total_orders', 'total_spend', 'avg_order_value', 'avg_review_score', 'avg_delivery_days', 'pct_on_time', 'num_categories', 'customer_age_days', 'has_repeat_orders', 'avg_days_between_orders']
['total_orders', 'total_spend', 'avg_order_value', 'max_order_value', 'min_order_value', 'customer_age_days', 'avg_review_score', 'avg_delivery_days', 'pct_on_time', 'num_categories', 'has_repeat_orders', 'avg_days_between_orders', 'avg_freight_ratio', 'avg_delivery_delta', 'gave_low_review', 'gave_high_review', 'has_review', 'review_missing', 'is_consumable_category', 'spend_tier']

# ── HEADER ───────────────────────────────────────────────────
st.title("🛍️ Olist Customer Intelligence System")
...
with tab2:
    ...
    The model is an **XGBoost classifier** (AUC-ROC: 0.812, Recall: 86.5%) trained on
    20 customer-level features. Top drivers: delivery lateness, freight ratio, order value.
    """)
    st.markdown("---")
    ...
    with col2:
        ...
        avg_delivery_days = st.slider("Avg Delivery Time (days)", 1, 100, 12)
        avg_delivery_delta = st.slider("Delivery Delta (actual − promised days)", -20, 60, 0,
                                        help="Positive = delivered late. Negative = delivered early.")
        pct_on_time = st.slider("% Orders Delivered On Time", 0.0, 1.0, 0.75, step=0.05)
...
with tab3:
    ...
    with col1:
        ...
        risk_threshold = st.slider(
            ...
            help="0.70 = High Risk threshold used in the business quantification"
        )
